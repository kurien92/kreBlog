...
public class SecuredObjectDao {
	...
	/**
	 * URL 형식인 보호자원 - Role 맵핑 정보를 조회하는 default 쿼리이다.
	 */
	...
	/**
	 * method 형식인 보호자원 - Role 맵핑 정보를 조회하는 default 쿼리이다.
	 */
	...
	/**
	 * pointcut 형식인 보호자원 - Role 맵핑 정보를 조회하는 default 쿼리이다. 
	 */
	...
	/**
	 * 매 REQUEST마다 best matching url 보호자원 - Role 맵핑정보를 얻기위한 default 쿼리이다.
	 */
	
	public static final String DEF_REGEX_MATCHED_REQUEST_MAPPING_QUERY_MYSQL =
			"SELECT A.RESOURCE_PATTERN AS URI, B.AUTHORITY AS AUTHORITY "
			+ "FROM SECURED_RESOURCE A, SECURED_RESOURCE_AUTHORITY B "
			+ "WHERE A.RESOURCE_ID = B.RESOURCE_ID "
			+ "AND A.RESOURCE_ID = "
	...
	/**
	 * Role의 계층(Hierarchy) 관계를 조회하는 default 쿼리이다.
	 */
	...
	public LinkedHashMap<Object, List<ConfigAttribute>> getRolesAndResources(String resourceType) throws Exception {
		...
		while (itr.hasNext()) {
			...
			// url인 경우 RequestKey 형식의 key를 Map에 담아야 함
			...
			// 이전 resourceKey에 현재 새로 계산된 Role 맵핑 리스트로 덮어쓰게 됨.
	...
	public List<ConfigAttribute> getRegexMatchedRequestMapping(String url) throws Exception {
		// best regex matching - best 매칭된 Uri에 따른 Role List 조회,
		// DB 차원의 정규식 지원이 있는 경우 사용 (ex. hsqldb custom function, mysql regexp_like 등)
		...
		// 같은 Uri에 대한 Role 맵핑이므로 무조건 configList에 add함
		while(itr.hasNext()) {
			tempMap = itr.next();
			configList.add(new SecurityConfig((String)tempMap.get("authority")));
		}
